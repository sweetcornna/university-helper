...
请只对本仓库 GitHub Releases 下载的官方安装包执行这条命令。
...
# Linux / macOS / WSL2
bash scripts/deploy_server.sh --tag 1.4.6 -y                 # 本机：http://localhost:8080
...
脚本也接受 `v1.4.6`，会自动规范化为 GHCR 镜像 tag `1.4.6`。加 `--build`
...
Release 镜像 tag 不带前缀 `v`（例如 `1.4.6`，不是 `v1.4.6`）。部署脚本两种
写法都接受。
